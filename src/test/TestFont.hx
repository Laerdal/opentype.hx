...
class TestFont extends buddy.BuddySuite {
    public function new() {


        var font;

        final fGlyph = new Glyph({name: 'f', unicode: 102, path: new Path(), advanceWidth: 1});
        final iGlyph = new Glyph({name: 'i', unicode: 105, path: new Path(), advanceWidth: 1});
        final ffGlyph = new Glyph({name: 'f_f', unicode: 0xfb01, path: new Path(), advanceWidth: 1});
        final fiGlyph = new Glyph({name: 'f_i', unicode: 0xfb02, path: new Path(), advanceWidth: 1});
        final ffiGlyph = new Glyph({name: 'f_f_i', unicode: 0xfb03, path: new Path(), advanceWidth: 1});
    ...
    /*
        describe('Font constructor', function() {
            ...
            it('tables definition shall be serialized', function() {
                final os2 = font.toTables().tables.find(table => table.tableName == 'OS/2');
        ...
        */
        describe('stringToGlyphs', function() {
            it('must support standard ligatures', function() {
                font.stringToGlyphs('fi').should.containAll([fGlyph, iGlyph]);
